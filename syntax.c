...
void doMagic() {
    ...
    int is_func = 0;
    char* func_num = NULL;
    BSTNodePtr* global_symtable = malloc(sizeof(struct BSTNode) * 10);        // global symtable storing the function ids
    BSTInit(global_symtable);
    BSTNodeLocalPtr *local_symtable = malloc(sizeof(struct BSTNode) * 10);       // local symtable storing the param ids inside a function
    BSTNodeContentPtr* content = malloc(sizeof(struct BSTNodeContent) * 10);
    BSTNodeContentLocalPtr* l_content = malloc(sizeof(struct BSTNodeContent) * 10);
    ...
    while (global_token.type != ss_eof) {
        ...
        if ((is_func == 1) && (global_token.type == s_id)) {        // push the function id into the global symtable
            ...
            content->declared = 1;
            content->defined = 1;
            content->ID = global_token.content;
            content->value = 0;
            BSTInsert(global_symtable, *content);
            BSTInitLocal(local_symtable);
            ...
            puts(func_num);
            l_content->type = NULL;
            l_content->declared = 1;
            l_content->defined = 1;
            l_content->ID = global_token.content;
            l_content->value = 0;
            BSTInsertLocal(local_symtable, *l_content);
        }
    }
